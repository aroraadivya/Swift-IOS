...
if let area = calculateArea(x: 5.0, y: 10.0) {
...
if let sum = add(5, 10) {
    ...
    print("Failed to add the numbers. One or both parameters were nil.")
}


if let sum = add(nil, 10) {
    print("The sum is \(sum).")
} else {
    print("Failed to add the numbers. One or both parameters were nil.")
}
...
if let user = createUser() {
    print("User created: \(user.firstName) \(user.lastName), email: \(user.email)")
} else {
    print("Failed to create user. Please ensure all fields are filled out.")
}
